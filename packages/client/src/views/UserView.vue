...
if (!isLogin.value) {
  openLogin()
    .catch(() => routers.push({ name: HOME_PAGE_NAME }))
}
...
const disabled = computed(() => {
  ...
  if (modelDataRef.value) {
    return !Object
      .keys(omit(modelData, 'email'))
      .every(key => modelDataRef.value[key as keyof typeof modelData])
  }
  else {
    return true
  }
  ...
  <n-layout>
    ...
    >
      ...
      <n-form-item path="phoneNum" label="手机号">
        <n-input v-model:value="modelDataRef.phoneNum" :maxlength="11" pattern="[0-9]" />
      </n-form-item>
